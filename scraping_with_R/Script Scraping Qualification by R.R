...
while (season <= max_season) {
  #Inicio de comando de navegación
  ...
  titulo <- page %>%
    ...
    {
      sapply(strsplit(., " ∙"), "[[", 1)
    } %>% #El nombre del título se hace innecesario
  ...
  calificaciones <- page %>%
    ...
    {
      gsub("\\Rate", "", .)
    } %>% #Eliminar Rate del contenido
    {
      gsub("\\(", "", .)
    } %>% #Eliminar el primer paréntesis del contenido
    {
      gsub("\\)", "", .)
    } #Eliminar el segundo paréntesis del contenido
